...
## 📍 当前进度快照（更新于 2026-06-17）

四个核心页面全部完成，已切真实后端 API（`VITE_USE_MOCK=false`），全链路实测跑通：填表 → 生成 → 轮询 → 结果 → 详情。v1.1 体验增强进行中：M9 动效已完成，结果页改版 + 状态找回已落地，表单输入已结构化对齐后端，M7 POI 详情待后端接口。

| 页面 | 状态 | 数据来源 |
|------|------|----------|
| 首页（表单） | ✅ 完成 | 真实提交，城市对齐后端 10 城；输入已结构化（节奏/人数/日期）+「继续上次行程」入口 |
| Loading（登机牌主题） | ✅ 完成 | 真实轮询，9 阶段→4 步进度映射；失败有表单友好文案 + 原地重试 |
| 结果页（拍立得改版 + 暖玻璃） | ✅ 完成 | 真实 `TripResult`（标题/摘要/标签/节奏/通勤） |
| 详情页（三栏 Dashboard） | ✅ 完成 | 行程/地图/通勤=真实；预算/天气/时刻=占位（已标「示意」） |

**近期迭代（2026-06-17）—— 表单结构化 + 流程打磨**：
- **表单对齐后端**（commit `51de736`）：节奏滑块映射为 preferences 标签（轻松/适中/紧凑，后端已消费）；新增人数控件并真实发送 `people_count`；补发 `start_date`/`end_date`（后端已消费）；notes 只发用户真实输入（不再拼接节奏/预算/多城市）；预算滑块保留但不入参（后端远期）；多城市开关置灰「敬请期待」（后端暂单城）；勾亲子且不足 2 人自动顶到 2 人，化解「1人带亲子」悖论；失败/返回首页从 sessionStorage 回填表单。
- **流程死角修复**（commit `57ddb46`）：「继续上次行程」加 7 天过期检查；地图加载失败由空白改为可见占位 + 重试；分享/收藏/行程灵感死按钮统一置灰「即将上线」。
- **失败体验**（commit `a2c7110`）：error_code → 表单场景文案映射（后端为自然语言写的「换个说法」类文案不再透传给表单用户）；失败态加「再试一次」同参原地重试。

**前期迭代（2026-06-15）**：结果页拍立得改版（`7742ce3`）、状态找回（`481e09b`）、占位透明化「示意」标记。
...
- **输入字段边界**：preferences/notes 已结构化打通；`people_count`、节奏标签、日期后端已消费；预算无可靠数据源故前端保留 UI 但不入参
...
> ✅ **输入字段已打通（2026-06-17）**：`people_count`、节奏标签（轻松/适中/紧凑）、亲子（留在 preferences）、notes、`start_date`/`end_date` 前后端已对齐消费，不再是待办。下表为剩余的**数据/接口**类待办。

| # | 待办 | 当前处理 | 谁来做 | 优先级 |
|---|------|----------|--------|--------|
| 1 | **天气数据** | 占位 `mockWeather()`，隔离在 `services/mockBudget.ts`，UI 已标「示意」 | 后端按 city+日期拉天气（带 key+缓存）并入 `TripResult.weather`；日期字段前端已备好 | 高 |
| 2 | **预算明细** | 占位 `mockBudget()`（按天数缩放），UI 已标「示意」；前端预算滑块保留但不入参 | 后端远期汇总门票/餐饮/住宿/交通真实账单 | 中 |
| 3 | **POI 详情接口** | `PlaceDetailModal` 已完成（展示结果已有字段），已移除 404 调用，等接口增强 | 后端新增 `GET /api/places/{id}`（图片/开放时间/门票/tips，字段全可空） | 中 |
...
**部署提醒**：线上为 Docker nginx（`kakarot-stack-nginx-1`），bind-mount `/opt/kakarot/travel-web/dist`。`vite build` 删 dist 重建会换 inode 导致容器看不到新产物 —— 每次部署后需 `docker restart kakarot-stack-nginx-1`。服务器构建需先建 `.env.local`（真实高德 key），否则 `.env.production` 的空 key 会 shadow 掉。

---

## 🧹 持续打磨 Backlog（前端独立可做，不绑版本号）

> 与上方"大版本里程碑"互补：里程碑管**功能**（多依赖后端），Backlog 管**打磨**（前端随时能做，做一个划一个）。
> 来源：2026-06-17 三份前端 skill 复评（impeccable 技术审计 13/20、taste-skill 设计语言、frontend-design-ui-ux 流程）+ 历次讨论。
> 优先级：P1 影响体验/可信度，P2 质量提升，P3 锦上添花。

### 已完成（本轮已做，留档）
- [x] 流程死角：「继续上次行程」过期检查、地图失败占位、死按钮置灰（commit `57ddb46`）
- [x] 失败体验：表单友好文案 + 原地重试（commit `a2c7110`）
- [x] em-dash → 普通标点（UI 文案）
- [x] 占位透明化：预算/天气「示意」标记

### 无障碍 / a11y（impeccable）
- [ ] **P1** 人数步进按钮 40px → 44px（`h-10 w-10` → `h-11 w-11`），满足触控目标
- [ ] **P2** PlaceDetailModal 加 Esc 关闭 + 焦点管理
- [ ] **P2** 详情页移动 Tab 加 `role="tab"`/`role="tablist"`/`aria-selected`
- [ ] **P2** PlanningPage 轮询进度加 `aria-live="polite"`，屏幕阅读器可感知

### 主题 / token（impeccable）
- [ ] **P2** 收敛硬编码 hex（滑块 `#fb923c`/`#f3f4f6`、SVG `#0f766e`、BoardingPass 等）到 token / CSS 变量
- [ ] **P3** 暗色模式（目前完全无 `dark:` 支持）——评估是否值得做

### 设计语言 / 反 slop（taste-skill）
- [ ] **P2** 首页左侧 4 个功能卡是"图标圈+标题+描述"模板填充物 → 砍或换一句有信息量的话
- [ ] **P2** 首页浮动模糊光晕与"旅行手账"概念无关，是通用玻璃装饰 → 移除或替换为契合的元素
- [ ] **P2** `✦` 装饰滥用（跨 3 页 5 处）→ 收敛为单一品牌符号
- [ ] **P3** 表单卡是纯现代玻璃，手账隐喻没渗透进交互面 → 加纸张质感/笔记本边
- [ ] **P3** 拍立得卡旋转 hover 回正在产品场景略 gimmick → 评估是否减弱

### 性能（impeccable）
- [ ] **P2** 城市背景大图懒加载（当前全视口图 eager 加载）
- [ ] **P3** MapView 每次 day/activePlaceId 变化重调 `AMapLoader.load()` → 用 ref 缓存的 AMap 实例

### 流程 / 状态（frontend-design-ui-ux，剩余）
- [ ] **P2** 轮询静默吞网络错误 → 连续失败 N 次后内联提示"网络不稳定"
- [ ] **P2** 单方案自动跳转前会闪一下 ResultPage 标题 → 渲染前判断

### 产品逻辑（历次讨论，需前后端协同）
- [ ] **P3** "亲子"归位：从兴趣标签抽出做"同行人"独立维度（solo/couple/family/friends）——等后端做 people_count/同行人时一起
- [ ] **待后端** notes vs preferences 冲突优先级：后端 prompt 定"notes > preferences"
- [ ] **待后端** 节奏"紧凑/适中"关键词后端是否已识别（前端已发标签）
...
| 阶段 | 内容 | 状态 | 依赖 |
|------|------|------|------|
| **A** 布局还原 & 视觉拉齐 | 首页设计稿、Loading 登机牌、结果页拍立得改版、详情页三栏、品牌色/logo、入场动效 | ✅ 已完成 | 纯前端 |
| **B** 数据填充 | 真实行程/地图/通勤已接；预算明细、天气、景点图、真实时刻待后端（占位已标「示意」） | 🟡 部分完成（占位已隔离+透明化） | 后端 API（见上方待办 #1-5） |
...
**周期**：3-4 天 — 🟡 前端组件已就绪（展示结果已有字段），等后端接口增强

| 任务 | 说明 |
|------|------|
| PlaceDetailModal 组件 | 点击地点弹出详情浮层（✅ 已完成，现展示名称/类别/简介/经纬度） |
| POI 详情 API 对接 | `GET /api/places/{place_id}`（待后端，当前 404 已移除调用） |
| 信息展示 | 简介、开放时间、门票、tips（字段全可空，有则渲染） |
| 图片展示 | 图片轮播（如有） |
...
**周期**：2-3 天 — ✅ **已完成**（commit `3ec13b3` 体验增强 + `7742ce3` 结果页 stagger-fade）

| 任务 | 说明 |
|------|------|
| 页面转场动画 | React Router 页面过渡（`page-enter` 仅透明度，避免破坏 fixed 定位） |
| 卡片入场动画 | 结果卡片 stagger 入场（拍立得卡用 `stagger-fade` 仅淡入，不覆盖自带旋转） |
| 进度条动画 | 步骤切换动画 |
| 地图动画 | Day 切换时地图平滑移动（`setFitView`） |
