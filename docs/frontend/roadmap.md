...
## 📍 当前进度快照（更新于 2026-06-15）

四个核心页面全部完成，已切真实后端 API（`VITE_USE_MOCK=false`），成都全链路实测跑通：填表 → 生成 → 轮询 → 结果 → 详情，无报错。v1.1 体验增强进行中：M9 动效已完成，结果页改版 + 状态找回已落地，M7 POI 详情卡后端接口。

| 页面 | 状态 | 数据来源 |
|------|------|----------|
| 首页（表单） | ✅ 完成 | 真实提交，城市对齐后端 10 城；新增「继续上次行程」入口 |
...
**近期迭代（2026-06-15）**：
- **B-2 结果页改版**：方案卡重做为拍立得（手写标题/轻微旋转/hover 回正）+ 暖玻璃背景 + `stagger-fade` 依次淡入（commit `7742ce3`）
- **B-1 状态找回**：表单+jobId 持久化 sessionStorage（刷新不丢偏好）；最近行程落 localStorage，首页加「继续上次行程」一键找回（commit `481e09b`）。注：单条+单设备，跨设备同步待 v1.2 用户体系
- **占位透明化**：详情页预算/天气卡加灰色「示意」标记，避免 mock 数据被当真实值
...
| # | 待办 | 当前处理 | 谁来做 | 优先级 |
|---|------|----------|--------|--------|
| 1 | **天气数据** | 占位 `mockWeather()`，隔离在 `services/mockBudget.ts`，UI 已标「示意」 | 后端下版本并入 `TripResult.weather`（按行程日期+城市对齐） | 高 |
| 2 | **预算明细** | 占位 `mockBudget()`（按天数缩放），UI 已标「示意」 | 后端远期汇总门票/餐饮/住宿/交通真实账单 | 中 |
